...
let with_thick (g: gctx) (t: bool) : gctx =
  { g with thickness = t }
  
  let with_tlevel (g: gctx) (t_l: int) : gctx =
  { g with t_level = t_l }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  (if t then Graphics.set_line_width gc.t_level else Graphics.set_line_width 1)
...
let draw_rect (g: gctx) (p1: position) ((w, h): dimension) : unit =
  ...
  let(x1, y1)=ocaml_coords g p1 in
 Graphics.moveto x1 y1;
 Graphics.lineto (x1+w) y1;
 Graphics.lineto (x1+w) (y1-h);
...
let draw_ellipse (g: gctx) (p: position) (rx: int) (ry: int) : unit =
  ...
  let (x, y) = ocaml_coords g p in
  Graphics.moveto x y;
  Graphics.draw_arc x y rx ry 0 180;
  Graphics.draw_arc x y rx ry 180 360
