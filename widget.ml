...
List.fold_right (fun x acc -> hpair x acc) ws (space(0,0))

let vlist (ws: widget list) : widget = 
List.fold_right (fun x acc -> vpair x acc) ws (space(0,0))
...
let make_controller (v: 'a) : 'a value_controller =
  let r = {value =v ; listeners = []} in
  {
  add_change_listener =
  (fun f -> r.listeners<- f :: r.listeners);
  get_value = 
  (fun () -> r.value);
  change_value = 
  (fun n -> r.value <-n;
  List.iter (fun f -> f n) r.listeners)
  } 
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
 ...
 let str = {contents = s} in
 
   let w1 = 
   {repaint = 
   (fun (g: Gctx.gctx) ->   
   Gctx.draw_rect g (0,0) (30,30); 
   if c.get_value () then (Gctx.fill_rect g (0,0) (30,30)) else ());
    handle = 
    (fun (g: Gctx.gctx) (e: Gctx.event) ->
       (begin match (Gctx.event_type e) with
       |MouseDown -> 
       (if c.get_value () then (c.change_value false) else 
       (c.change_value true))
       |_ -> ()
       end)
       );
    size = 
    (fun () -> Gctx.text_size str.contents)
  } in 
  let w2 = 
  {repaint = 
   (fun (g: Gctx.gctx) ->   
   Gctx.draw_string g (0,10) str.contents);
    handle = 
    (fun (g: Gctx.gctx) (e: Gctx.event) ->
       (begin match (Gctx.event_type e) with
       |MouseDown -> 
       (if c.get_value () then 
       (c.change_value false) else 
       (c.change_value true))
       |_ -> ()
       end)
       );
...
(* slider function that makes a slider widget *)

let slider (x : int) (s: string): widget * int value_controller =
  let c = make_controller x in
  let str = {contents =s} in
  let w1 = 
  {repaint =
  (fun (g: Gctx.gctx) ->   
  Gctx.fill_rect g (0,0) (c.get_value(), 30));

  handle = 
(* makes a filled rectangle with a fixed height and a width of the mouse's x
coordinate *)
 ( fun (g:Gctx.gctx) (e: Gctx.event) ->
     let p  = Gctx.event_pos e g in
     let (x,y) =p in
    (begin match (Gctx.event_type e ) with
    |MouseDown -> (c.change_value x ;Gctx.fill_rect g (0,0) (c.get_value(), 30))
    |MouseDrag -> (c.change_value x ;Gctx.fill_rect g (0,0) (c.get_value(), 30))
    |MouseUp ->Gctx.fill_rect g (0,0) (c.get_value(), 30) 
    |_ -> ()
    end));
  size = 
  fun () -> Gctx.text_size str.contents
